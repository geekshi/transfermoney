...
public class ControllerUtil {
	...
	public static Object getAccounts(Map<String, Account> accounts){
		AccountsResponse response = new AccountsResponse();
		response.setTotalAccounts(accounts.size());
		List<AccountBrief> array = new ArrayList<>();
		for(Map.Entry<String, Account> entry : accounts.entrySet()){
			AccountBrief brief = new AccountBrief(entry.getKey(), 
					entry.getValue().getAccountName(), Constants.ACCOUNTS + "/" + entry.getKey());
			array.add(brief);
		}
